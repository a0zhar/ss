...
void Menu::monitorButtons() {
	if (open) {
		...
		// Set all of the Controller Button Interaction Flags to false
		optionPress = false;
		leftPress = false;
		rightPress = false;
		leftHold = false;
...
void Menu::drawText(char* text, Vector2 pos, int size, Font gamefontfamily, char* color, char* alignment, bool outline) {
	char buffer[512];

	sprintf(buffer,
		"<TEXTFORMAT INDENT='0' LEFTMARGIN='0' RIGHTMARGIN='0' LEADING='0'>"
		"<FONT FACE='$%s' COLOR='#%s' SIZE='%i'><P ALIGN='%s'>%s</P></FONT>"
		"</TEXTFORMAT>",
		fontList[gamefontfamily], color, size, alignment, text
	);
...
void Menu::run() {
	...
	// Draw The Text which represents the Menu Title
	drawText(
		"Serotonin",      // Text to be displayed
		{ 0.660f, 0.1f }, // Text position (x, y)
		45,               // Text letter size
		Redemption,       // Text Font family
		"004B93",         // Text Color
		"CENTER",         // Text alignment
		true              // Outlined(Stroke)?
	);


	// Draw the Body of our Menu (including its initial Options)
