...
// ------------------------------------
typedef unsigned int uint; // Unsigned integer type with 32 bits. Used for representing positive integers.
typedef unsigned long int size_t;
typedef void* Void;
typedef int Any; 		   // General-purpose type that can represent any data. Usage may vary depending on context.
typedef int Hash; 		   // hash value, which is often used for fast lookups or comparisons of strings.
typedef int BOOL; 	       // boolean value, where 0 means false and any non-zero value means true.
typedef int Entity; 	   // in-game entity, such as characters, objects, or vehicles.
typedef int Player; 	   // player entity in the game.
typedef int FireId; 	   // identifier for fire effects in the game.
typedef int Ped; 	  	   // pedestrian (NPC or character) in the game.
typedef int Vehicle; 	   // vehicle in the game.
typedef int Cam; 	       // camera entity used for in-game cameras or views.
typedef int CarGenerator;  // car generator, used to spawn vehicles in the game world.
typedef int Group; 		   // group of entities that may have shared behavior or properties.
typedef int Train;		   // train entity in the game.
typedef int Pickup;        // pickup item that players can collect in the game.
typedef int Object;        // generic object entity in the game.
typedef int Weapon;        // weapon item that players can use in the game.
typedef int Interior;      // interior location or space within the game world.
typedef int Blip;	       // visible map marker or indicator on the game's minimap.
typedef int Texture; 	   // texture resource used for rendering in the game.
typedef int TextureDict;   // collection of textures in a dictionary.
typedef int CoverPoint;    // cover point location that entities can take cover behind.
typedef int Camera;        // camera entity used for in-game cameras or views.
typedef int TaskSequence;  // sequence of tasks or actions for an entity to perform.
typedef int ColourIndex;   // index value for a color palette in the game.
typedef int Sphere; 	   // 3D sphere used for collision detection or spatial calculations.
typedef int ScrHandle; 	   // script handle used to reference various game objects or data.
// Missing types (more context needed to provide specific comments)
// --------------
typedef Any AnimScene;     // animation scene or sequence.
typedef Any Prompt;        // user prompt or interaction in the game.
typedef Any ItemSet;       // set of items or entities.
typedef Any PersChar;      // persistent character or entity in the game.
typedef Any PopZone;  	   // population zone or area in the game world.
typedef Any PropSet;       // set of properties or attributes for entities.
typedef Any Volume;        // volume or space within the game world.
